...
class GameConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        if self.game.second_player is not None and not cache.get(f'game_{self.group_name}'):
            ...
            try:
                self.game.first_player.wallet.transaction(self.game.bet * -1,'❌ TicTacToe', f'Deposit #{self.game.id}')
                self.game.second_player.wallet.transaction(self.game.bet * -1, '❌ TicTacToe', f'Deposit #{self.game.id}')
    ...
    def receive(self, text_data: str=None, bytes_data=None):
        ...
        if winner:
            winner.wallet.transaction(self.game.bet * 2,'❌ TicTacToe', f'Win #{self.game.id}')
            event = {
                'type': 'update_map_handler',
                'map': map,
